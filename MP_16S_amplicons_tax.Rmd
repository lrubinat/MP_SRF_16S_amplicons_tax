...
tb16_tax_occur_min8522 <- tb16_tax_occur[,colSums(tb16_tax_occur) >= 5844]
dim(tb16_tax_occur_min8522)
...
max(colSums(tb16_tax_occur_min8522)) 
# max: 936570
```
...
amplicons_per_sample<-colSums(tb16_tax_occur_min8522)
amplicons_per_sample[which(colSums(tb16_tax_occur_min8522)>900000)]
```

Overall reads per sample:

``` {r reads_per_sample_overview4, echo=FALSE}
plot(sort(colSums(tb16_tax_occur_min8522)), pch=19, xlab="sample", ylab="reads per sample", cex=0.9)
```
...
tb16_tax_occur_ss8522_no_cero<-tb16_tax_occur_ss8522[,-(which(colSums(tb16_tax_occur_ss8522)==0))]
dim(tb16_tax_occur_ss8522_no_cero)
...
Datasets summary:
dim(tb16_tax) --> 47144   131
dim(tb16_tax_occur) --> 47144   124
dim(tb16_tax_occur_ss8522_no_cero) --> 123 27067

Let's add the taxonomic classification to the left OTUs by merging "tb16_tax_occur_ss8522_no_cero" with "tb16_tax".

```{r merge_tables, echo=FALSE}
tb16_tax_occur_ss8522_no_cero_t<-t(tb16_tax_occur_ss8522_no_cero)
tb16_ss8522_tax<-merge(tb16_tax_occur_ss8522_no_cero_t,tb16_class, by="row.names")

dim(tb16_ss8522_tax)
tb16_ss8522_tax[1:5,1:5]

#fix OTU_no as new row
rownames(tb16_ss8522_tax)=tb16_ss8522_tax$Row.names

#add OTU_no as rowname
rownames.tb16_ss8522_tax<-tb16_ss8522_tax[,1]
tb16_ss8522_tax<-tb16_ss8522_tax[,-1]
#colnames(tb16_ss8522_tax, do.NULL=F)

dim(tb16_ss8522_tax)
tb16_ss8522_tax[1:5, 1:5]

#sort by OTU_no (split rowname, introduce no. into column "OTU_no" and sort)
tb16_ss8522_tax["OTU_no"] <- NA
tb16_ss8522_tax$OTU_no <- sapply(strsplit(rownames(tb16_ss8522_tax),split= "\\_"),'[',2)
tb16_ss8522_tax$OTU_no <- as.numeric(as.character(tb16_ss8522_tax$OTU_no))
tb16_ss8522_tax_sorted<-tb16_ss8522_tax[order(tb16_ss8522_tax$OTU_no, decreasing = FALSE), ]

dim(tb16_ss8522_tax_sorted)
tb16_ss8522_tax_sorted[1:5,1:5]
```

```{r select_phototrophs, echo=FALSE}
tb16_phototrophs <- tb16_ss8522_tax_sorted[which(tb16_ss8522_tax_sorted$MAS_plus_BM_plus_SILVA_class != "NA"),]
